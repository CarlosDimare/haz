    ...
    commands: [
      {
        ...
        title: "Enviar mensaje de diálogo",
        category: "Dialog",
        run: confirm,
      },
    ],
    ...
    <box paddingLeft={2} paddingRight={2} gap={1}>
      ...
      <box gap={1}>
        ...
        <Show when={props.busy}>
          <Spinner color={theme.textMuted}>{props.busyText ?? "Trabajando..."}</Spinner>
        </Show>
      </box>
      <box paddingBottom={1} gap={1} flexDirection="row">
        <Show when={!props.busy} fallback={<text fg={theme.textMuted}>procesando...</text>}>
          <Show when={submitShortcut()}>
            <text fg={theme.text}>
              {submitShortcut()} <span style={{ fg: theme.textMuted }}>enviar</span>
            </text>
          </Show>
        </Show>
      </box>
